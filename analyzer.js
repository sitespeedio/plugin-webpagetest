        ...
        for (let run = 1; run <= wptOptions.runs; run++) {
          ...
          promises.push(
            getScreenshotImage(id, {
              ...
              .catch(e => {
                log.warn(
                  `Couldn't get screenshot for id ${id}, run ${run} from the WebPageTest API with the error ${e.message} (url = ${url})`
                );
              })
          );
          ...
          promises.push(
            getWaterfallImage(id, {
              ...
              .catch(e =>
                log.warn(
                  `Couldn't get waterfall for id ${id}, run ${run} from the WebPageTest API with the error: ${e.message} (url = ${url})`
                )
              )
          );
          ...
          promises.push(
            getWaterfallImage(id, {
              ...
              .catch(e =>
                log.warn(
                  `Couldn't get connection waterfall for id ${id}, run ${run} from the WebPageTest API with the error: ${e.message} (url = ${url})`
                )
              )
          );
          ...
          if (wptOptions.timeline) {
            promises.push(
              getChromeTraceData(id, {
                ...
                .catch(e =>
                  log.warn(
                    `Couldn't get chrome trace for id ${id}, run ${run} from the WebPageTest API with the error: ${e.message} (url = ${url})`
                  )
                )
            );
          }
